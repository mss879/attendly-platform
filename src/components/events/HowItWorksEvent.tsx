...
// Per-event booking journey (generalized from the original Bradby
// HowItWorks — copy and CTA are driven by the event's price and slug).

const iconClass = "h-7 w-7";

function buildSteps(priceLabel: string) {
  return [
    {
      title: "Pick your seats",
      text: "Fill in your details, then choose your numbered seats on the live seating plan — taken seats grey out as they go.",
      badge: "Step 1",
    ...
    },
    {
      ...
      text: `Transfer ${priceLabel} per seat to the bank account shown at checkout and upload your payment slip to confirm.`,
      badge: "Step 2",
...
export function HowItWorksEvent({
  priceLabel,
  bookHref,
  completed,
}: {
  ...
  const steps = buildSteps(priceLabel);
      ...
      const ctx = gsap.context(() => {
        ...
        gsap.utils.toArray<HTMLElement>(".how-step").forEach((card) => {
          ...
          gsap.fromTo(
            ...
            { y: 0, autoAlpha: 1, scale: 1, duration: 0.7, ease: "power3.out", scrollTrigger: trigger, clearProps: "all" }
          );
          gsap.fromTo(
            card.querySelector(".step-num"),
            { scale: 0.3, autoAlpha: 0 },
            { scale: 1, autoAlpha: 1, duration: 0.6, delay: 0.1, ease: "back.out(2)", scrollTrigger: trigger, clearProps: "all" }
          );
          gsap.fromTo(
            card.querySelectorAll(".step-draw"),
            { drawSVG: "0%" },
            { drawSVG: "100%", duration: 0.9, delay: 0.25, stagger: 0.12, ease: "power2.inOut", scrollTrigger: trigger }
          );
        });
        ...
        <ol className="relative grid gap-8 sm:grid-cols-3 sm:gap-5">
          {steps.map((step, i) => (
            <li key={step.title} className="how-step flex gap-4 sm:block">
              ...
              <div className="step-card relative mt-0 flex-1 overflow-hidden rounded-2xl bg-white/40 p-5 shadow-lg shadow-orange-950/5 ring-1 ring-white/60 backdrop-blur-md transition-all duration-300 hover:scale-[1.01] hover:bg-white/55 hover:shadow-xl hover:shadow-orange-950/5 sm:mt-5 sm:text-center">
                <span aria-hidden className="pointer-events-none absolute inset-0" style={{ background: step.wash }} />
                <div className="relative">
                  <span className={`inline-flex rounded-full px-2.5 py-1 text-[11px] font-bold ${step.pill}`}>
                    {step.badge}
                  </span>
